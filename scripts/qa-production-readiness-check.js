...
check(files.index.includes("v=20260719-4"), "index should bump static asset cache version after feedback-export changes");
...
check(files.app.includes("copyFeedbackSummary") && files.app.includes("复制脱敏反馈"), "app should provide a sanitized feedback copy path");
check(files.app.includes("feedbackReasons") && files.app.includes("症状没识别准"), "app should collect detailed feedback reasons");
check(files.app.includes("不包含用户原始症状输入"), "feedback summary should state that raw input is excluded");
